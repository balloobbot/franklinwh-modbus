...
M1_MD = 20  # inside model 1, the first component the settings poll reads
M502_STAT = 1100  # inside model 502, the first component the readings poll reads
M701_W = 80
...
async def test_a_timeout_on_the_first_component_raises(
    ...
    """
    unit.fail_read(M502_STAT, ModbusTimeoutError("silent device"))
    unit.read_events.clear()
...
async def test_a_silent_device_is_fatal_to_a_settings_poll_too(
    agate: AGate, unit: MockModbusUnit
) -> None:
    """The settings poll starts its own cycle, so nothing has answered yet.

    Contained, thirteen components would each pay a full timeout to learn the
    device is not talking.
    """
