...
All battery control relies on a mandatory, multi-step sequence of direct register writes. The application uses the raw `pymodbus` client to perform these actions, as high-level `sunspec2` model writes are ignored by the hardware.
...
sequenceDiagram
    ...
    Ctrl->>H: 2. Configure Mode (Write Reg 318 = 0) & Disable Reversion (Write Reg 326 = 0)
    H-->>Ctrl: Ack
    ...
    H-->>Ctrl: Return -1000
    Ctrl-->>App: Success, Command Sent: -1000W
```

---
## 2. Standby / Release Control Sequence

To return the battery to its native mode (e.g., Self-Consumption), control must be explicitly released.
